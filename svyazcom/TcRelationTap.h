...
class TcRelationTap : public Version {
    ...
    public:
        ...
        explicit TcRelationTap(const std::vector<std::string>& data) {
        ...
        std::string toString() const override {
            ...
            std::string tap_string =
                " tax_profile: " + std::to_string(tax_profile) +
                " service_profile: " + std::to_string(service_profile) + " tp_check: " + std::to_string(tp_check) +
                " check_mode: " + std::to_string(check_mode) + " tp_reprice: " + std::to_string(tp_reprice) +
                " transit_tap_mode: " + std::to_string(transit_tap_mode) + " transit_start: " + transit_start +
                " transit_end: " + transit_end + " transit_period: " + std::to_string(transit_period) +
                " gen_rap: " + (gen_rap ? "1" : "0") + " transit_rap: " + (transit_rap ? "1" : "0") +
                " pass_rc_less_ic: " + (pass_rc_less_ic ? "1" : "0") +
                " iot_threshold: " + std::to_string(iot_threshold) + " iot_debug: " + std::to_string(iot_debug);
